...
export const missingTrust=():Claim[]=>[
 ...
 {id:"external",layer:"External",label:"Confidential external replay",status:"unsupported",detail:"No external verifier result has been obtained."},
...
export async function verifyBrowserEvidence(value:unknown):Promise<VerificationReport>{
 ...
 return {format:"synapsevm.verification-report.v1",verifierId:"synapsevm-browser-module-verifier-v1",subject:model.name+"@"+model.version,mode:"Local module replay",outcome:claims.some(c=>c.status==="fail")?"MISMATCH":"MATCH",claims,identities:{model:e.commitments.model,input:e.commitments.input,stateBefore:e.commitments.stateBefore,stateAfter:e.commitments.stateAfter},scope:"A fresh instance of the same JavaScript runtime reproduced the disclosed module execution. This does not establish independent implementation, device identity, sensor truth, or actuator behavior."};
}
