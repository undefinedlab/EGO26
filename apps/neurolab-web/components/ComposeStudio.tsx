...
function initial():Draft{const graph=template("brake");return {graph,positions:layout(graph),modules:{}};}
...
export function ComposeStudio({mode="compose"}:{mode?:"compose"|"compile"|"simulate"}){
 ...
 const [fileName,setFileName]=useState("EmergencyBrake-0.1.0.synapse"),[savedPresets,setSavedPresets]=useState<CompilePreset[]>([]),[saveOk,setSaveOk]=useState(false),[inspectorOpen,setInspectorOpen]=useState(true),[compileLogs,setCompileLogs]=useState<CompileLog[]>([]);
